        ...
        <div key={i} style={{ display: 'flex' }}>
          {(colour==='b' || colour==='black'?row.slice().reverse():row).map((square, j) => {
            ...
            <div onClick={() => {
              ...
              key={j} className={`w-10 h-10 md:h-16 md:w-16 lg:h-18 lg:w-18  xl:h-24 xl:w-24 flex justify-center border items-center ${(i + j) % 2 !== 0 ? 'bg-slate-600' : 'bg-slate-300'} relative`}>
